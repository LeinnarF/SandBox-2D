...
void Element::Draw(Grid& grid, int y, int x, int type)
{

	if (grid.checkCell(y, x, 1))
		DrawRectangle(x * grid.getCellsize(), y * grid.getCellsize(), grid.getCellsize(), grid.getCellsize(), BEIGE);

	else if (grid.checkCell(y, x, 2))
		DrawRectangle(x * grid.getCellsize(), y * grid.getCellsize(), grid.getCellsize(), grid.getCellsize(), BLUE);
		...
		DrawRectangle(x * grid.getCellsize(), y * grid.getCellsize(), grid.getCellsize(), grid.getCellsize(), ORANGE);
}

void Element::Update(Grid& grid, int y, int x, int type)
{
	if (grid.checkCell(y, x, 1))
		Sand(grid, y, x);
